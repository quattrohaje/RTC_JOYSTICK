...
color_blue       	equ 01h  ; Blue
color_green      	equ 02h  ; Green
color_cyan       	equ 03h  ; Cyan
color_red        	equ 04h  ; Red
color_magenta    	equ 05h  ; Magenta
color_brown      	equ 06h  ; Brown
color_light_gray 	equ 07h  ; Light Gray
color_dark_gray  	equ 08h  ; Dark Gray
color_light_blue 	equ 09h  ; Light Blue
color_light_green 	equ 0Ah ; Light Green
color_light_cyan 	equ 0Bh  ; Light Cyan
color_light_red  	equ 0Ch  ; Light Red
color_light_magenta 	equ 0Dh ; Light Magenta
color_yellow     	equ 0Eh  ; Yellow
color_white      	equ 0Fh  ; White
...
; Print the sign-in message
	...
	mov	dx,msg_signin		; Print welcome message
	call	print_string
...
; Validate date and time
	...
	mov	al,cs:[century+1]
	call	bcd_to_binary
;Q	cmp	al,20			; century must be between 19 and 20
;Q	ja	.reset_clock
;Q	cmp	al,19
;Q	jb	.reset_clock
	...
	mov	al,cs:[year+1]
	call	bcd_to_binary
;Q	cmp	al,99			; year must be between 0 and 99
;Q	ja	.reset_clock
	mov	cl,al			; save the year value in CL

	mov	al,cs:[month+1]
;Q	cmp	al,12			; month must be between 1 and 12
;Q	ja	.reset_clock
;Q	cmp	al,1
;Q	jb	.reset_clock
...
; check for February in a leap year
	...
	mov	al,cs:[date+1]
	call	bcd_to_binary
;Q	cmp	al,1			; day must be above or equal to 1
;Q	jb	.reset_clock
...
.check_date:
;Q	cmp	al,ah			; AL - current date
;Q	ja	.reset_clock		; AH - days in this month

	mov	al,cs:[day+1]
;Q	cmp	al,7			; day must be between 1 and 7
;Q	ja	.reset_clock
;Q	cmp	al,1
;Q	jb	.reset_clock
	mov	al,cs:[hours+1]
	call	bcd_to_binary
;Q	cmp	al,23			; hours must be between 0 and 23
;Q	ja	.reset_clock
	mov	al,cs:[minutes+1]
	call	bcd_to_binary
;Q	cmp	al,59			; minutes must be between 0 and 59
;Q	ja	.reset_clock
	mov	al,cs:[seconds+1]
	call	bcd_to_binary
;Q	cmp	al,59			; seconds must be between 0 and 59
;Q	ja	.reset_clock
...
; Print the RTC I/O port number
	...
	mov	dx,msg_rtc_port
	call	print_string
	mov	al, color_cyan		; cyan for IO port
	mov	ah, 0x07		; length IO oprt (0x0000) + 1
	call	print_char_colored	; set colored text
	mov	dx,msg_rtc_port_0x
	call	print_string
...
; Print current date and time
	...
	mov	dx,msg_rtc_time
	call	print_string
	mov	al, color_blue		; blue for date and time
	mov	ah, 0x14		; length date and time (0000-00-00 00:00:00) + 1
	call	print_char_colored	; set colored text
	mov	ah,cs:[century+1]
	mov	al,cs:[year+1]
...
; Detection JOYSTICKs
	mov	dx,msg_joystick
	call	print_string
  	mov dx, 0x201        ; Nastavení port adresy pro joystick
        in  al, dx           ; Načtení aktuálního stavu z portu
	and al, 0x0F
        cmp al, 0x0F
    	je  NoJoystick
	cmp al, 0x0E
	je  PrimaryJoystick
    	...
    	je  PrimaryJoystick
    	cmp al, 0x07
    	je  SecondaryJoystick
    	cmp al, 0x0B
    	je  SecondaryJoystick
    	jmp PrimaryAndSecondary
NoJoystick:
	mov	al, color_red		; red joystick
	mov	ah, 0x0B		; length date and time (No present) + 1
	call	print_char_colored	; set colored text

	mov	dx,msg_joystick_0
	call	print_string
     	jmp joy_done
PrimaryJoystick:
	mov	dx,msg_joystick_1
	call	print_string
	mov	al, color_green		; green joystick
	mov	ah, 0x02		; length date and time (A) + 1
	call	print_char_colored	; set colored text
	mov	al,'A'
	call    print_char
    	jmp joy_done
SecondaryJoystick:
	...
	call	print_string
	mov	al, color_green		; green joystick
	mov	ah, 0x02		; length date and time (B) + 1
	call	print_char_colored	; set colored text
	mov	al,'B'
	call    print_char
...
no_rtc:
	push	dx
	mov	al, color_red		; red ERROR
	mov	ah, 0x06		; length date and time (Error) + 1
	call	print_char_colored	; set colored text
	mov	dx,msg_error
	call	print_string
...
;=========================================================================
; print_char_colored - print character with color using BIOS
; Input:
; AH - number char colored + 1
; AL - number color
;  - **Black:** 0x00
;  - **Blue:** 0x01
;  - **Green:** 0x02
...
print_char_colored:
    ...
    push cx
    mov	ch,0
    mov cl, ah          ; repeat count (1 to print once)
    mov bl, al		; color
    mov al, ' '
    mov ah, 09h        ; BIOS teletype output function
...
msg_signin	db	'RTC and Joystick Driver, v 1.0.0, 2024 Quattro', 0Dh, 0Ah,'$'
msg_rtc_port	db	' I/O port : $'
msg_rtc_time	db	' Time     : $'
msg_joystick 	db 	' Joystick : $'
msg_rtc_dse_ena	db	'; DSE enabled$'
msg_rtc_dse_dis	db	'; DSE disabled$'
msg_rtc_batt	db	'Warning: The RTC battery is bad$'
msg_rtc_inval	db	07h, 'Warning: Invalid date or time. '
		db	'Setting to default values', 0Dh, 0Ah, '$'
msg_error	db	'Error$'
msg_no_rtc	db	07h, ': No RTC detected at the I/O port$'
msg_inv_port	db	07h, ': Invalid port number$'
msg_inv_arg	db	07h, ': Invalid command line argument'
...
msg_joystick_0  db	'No present$'
msg_joystick_1  db	'Present $'
msg_rtc_port_0x	db 	'0x$'
